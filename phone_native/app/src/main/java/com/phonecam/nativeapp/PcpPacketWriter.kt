...
/**
 * PcpPacketWriter —— phone_native/ 批次 3.2.0.3a PCP 24 字节头打包器
 *
 * 作用: 把 H.264 NALU 字节 + 元数据 (sequence/pts/keyframe) 打成 PCP 协议包
 *       格式: [24 字节头 (小端)][NALU payload]
 *       等同于 desktop/receiver.py 的 HEADER_STRUCT = struct.Struct('<4sBBBBIQI')
 *
 ...
 *   │ 12      8     pts          u64 (微秒)            │
 *   │ 20      4     payload_len  u32 (字节数)          │
 *   ├──────────────────────────────────────────────────┤
 *   │ 24      N     payload      H.264 NALU 字节        │
 *   └──────────────────────────────────────────────────┘
 *   总头 24 字节, 所有字段小端序
 *
 * 范围 (批次 3.2.0.3a):
...
object PcpPacketWriter {
    ...
    /**
     * 构造 24 字节 PCP 头 (小端序, 与 Python struct.Struct('<4sBBBBIQI') 字符级一致)
     *
     * @param sequence  帧编号 (u32, 从 0 开始, 溢出回卷)
     * @param ptsUs     帧时间戳 (u64, 微秒, 从 session 起点算)
     ...
     * @return 24 字节的 ByteArray, 小端序, 可直接写 socket.getOutputStream()
     */
    fun buildHeader(
        sequence: Int,
        ptsUs: Long,
        ...
        return ByteBuffer.allocate(HEADER_SIZE)
            ...
            .apply {
                ...
                put(VERSION)            // 4      version (1 字节)
                put(type)               // 5      type    (1 字节)
                put(codec)              // 6      codec   (1 字节)
                put(flags)              // 7      flags   (1 字节)
                putInt(sequence)        // 8..11  sequence u32 (4 字节)
                putLong(ptsUs)          // 12..19 pts      u64 (8 字节)
                putInt(payloadLen)      // 20..23 payload_len u32 (4 字节)
            }
            .array()
    }

    /**
     * 构造完整 PCP 包: [24 字节头][payload]
     *
     * @param sequence   帧编号
     * @param ptsUs      帧时间戳 (微秒)
     ...
     * @return 长度 = 24 + payload.size 的 ByteArray, 可直接 socket.getOutputStream().write()
     */
    fun buildPacket(
        sequence: Int,
        ptsUs: Long,
