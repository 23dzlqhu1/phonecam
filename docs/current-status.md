...
> 最后更新：2026-06-19 BUG-013 PC 低延迟修复（frame age + NV12 直接路径）

## 事实来源

- 2026-06-19 BUG-013 PC 低延迟修复：(1) `receive_time` 统一为毫秒 (2) `DecodeWorker::decodeFrame` 加 frame age 测量 + >500ms 丢帧追实时 + decoder flush (3) 非 NV12 格式不再走 RGB24/QImage 往返，改为 `sws_scale(srcFmt→NV12)` 直接转换后走 NV12 rotation+scale 快速路径 (4) `[COMPOSER] FALLBACK` 日志改为前 10 帧 + 每 300 帧。`build_quick.bat` 通过；帧率/延迟验收待人工。
- 2026-06-19 BUG-013 PC fast path 格式假设修复：`composeFromDecodedFrame` NV12 检测 + `fallbackToQImageCompose`。`decodeFrame` 用 `av_new_packet+memcpy`。`build_quick.bat` 通过。
