...
> 最后更新：2026-06-19 BUG-013 关闭 + 稳定基线 `bug-013-fixed`

## 事实来源

- 2026-06-19 BUG-013 关闭：花屏/延迟/transform 问题全部修复并验收通过。最终根因链：Android 端 encoder 尺寸错配 + lifecycle 竞态（代际隔离修复）→ PC 端 NV12 compose 格式假设 + 无界队列延迟（直接 NV12 路径 + frame age 丢帧修复）→ transform 触发 legacy 分支（NV12 mirror/flip 统一路径修复）。稳定基线 tag: `bug-013-fixed` @ `72b33d7`。
